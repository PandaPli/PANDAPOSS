 ...
 * /pedir/vercarta
 *
 * - Con un solo local → redirige directo a /pedir/vercarta/[slug]
 * - Con múltiples locales → muestra selector
 * - ?sucursal=ID (legacy) → redirige a /pedir/vercarta/[slug]
 */
export default async function VerCartaPage({ searchParams }: Props) {
  ...
  if (sucursal) {
    ...
    if (found) redirect(`/pedir/vercarta/${createSlug(found.nombre)}`);
    notFound();
  }

  // Un solo local → redirigir directo
  if (sucursales.length === 1) {
    redirect(`/pedir/vercarta/${createSlug(sucursales[0].nombre)}`);
  }
        ...
        <div className="rounded-[2rem] border border-black/10 bg-white/90 p-8 shadow-[0_35px_100px_-60px_rgba(0,0,0,0.4)] backdrop-blur text-center">
          ...
          <div className="mt-8 space-y-3 text-left">
            {sucursales.map((item) => (
              <Link
                key={item.id}
                href={`/pedir/vercarta/${createSlug(item.nombre)}`}
