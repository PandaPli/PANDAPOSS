...
export function QRPageClient({ salas, estacionamientos: initialEstacionamientos, sucursales, isAdmin, sucursalId }: Props) {
  ...
  async function generarQRMesa(mesa: Mesa) {
    ...
    try {
      ...
      if (data.qr) setModal({ label: mesa.nombre, qr: data.qr, url: data.url });
  ...
  async function generarQREstacionamiento(spot: EstacionamientoItem) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { alert(data.error || "Error al generar el QR"); return; }
      if (data.qr) setModal({ label: `Estacionamiento ${spot.numero}`, qr: data.qr, url: data.url });
    } catch { alert("Error de red"); } finally { setLoadingId(null); }
  }
  ...
  const tabs: { key: Tab; label: string; icon: typeof QrCode }[] = [
    { key: "mesa", label: "Mesa QR", icon: UtensilsCrossed },
  ...
  return (
    <>
      {/* Tabs */}
      <div className="flex gap-1 bg-surface-bg border border-surface-border rounded-xl p-1">
        {tabs.map((t) => {
          const Icon = t.icon;
          return (
            <button
              ...
              className={cn(
                ...
                tab === t.key
                  ...
                  : "text-surface-muted hover:text-surface-text hover:bg-white/50"
              )}
            >
              <Icon size={16} />
              {t.label}
            ...
            <div className="flex gap-3 items-end flex-wrap">
              <div className="flex-1 min-w-[140px]">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-surface-border bg-white px-3 py-2 text-sm outline-none focus:border-purple-400"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
            {spots.map((spot) => {
              ...
              return (
                <div
                  key={spot.id}
                  className="card p-4 flex flex-col items-center gap-3 text-center group relative"
                >
                  <button
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-surface-text">QR — {modal.label}</h3>
                <p className="text-xs text-surface-muted mt-0.5">Listo para imprimir</p>
              </div>
              <button
                onClick={() => setModal(null)}
