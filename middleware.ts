...
const ROLE_ROUTES: Record<Rol, string[]> = {
  ADMIN_GENERAL: ["*"],
  RESTAURANTE: ["/panel", "/mesas", "/pedidos", "/ventas", "/productos", "/clientes", "/cajas", "/usuarios", "/rrhh", "/compras", "/reportes", "/configuracion", "/perfil", "/delivery", "/carta-qr", "/kiosko-admin", "/cupones", "/planes"],
...
export const config = {
  matcher: [
    "/((?!login|home|menu|pedir|vercarta|kiosko|track|visor|api/|_next/static|_next/image|favicon\\.ico|.*\\.png$|.*\\.jpg$|.*\\.svg$|.*\\.ico$).*)",
  ],
};
