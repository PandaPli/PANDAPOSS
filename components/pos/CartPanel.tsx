      ...
      {items.length > 0 && (
        <div className="border-t border-surface-border p-3 space-y-2.5">
          {modoCuentas && grupos.length > 0 ? (
            <div className="space-y-2">
              {/* Enviar cocina siempre disponible */}
              {items.some((i) => !i.guardado && !i.cancelado && !i.pagado) && (
                <button
                  onClick={onOrden}
                  disabled={ordenLoading}
                  className="flex w-full items-center justify-center gap-1.5 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700 transition hover:bg-amber-100 disabled:opacity-50"
                >
