...
const modules: AppModule[] = [
  ...
  { label: "Delivery",      href: "/delivery",    icon: <Bike            size={22} />, color: "bg-orange-600",  roles: ["ADMIN_GENERAL", "ADMIN_SUCURSAL", "DELIVERY"], featureKey: "delivery" },
  { label: "Carta QR",      href: "/carta-qr",   icon: <QrCode          size={22} />, color: "bg-purple-600",  roles: ["ADMIN_GENERAL", "ADMIN_SUCURSAL"],             featureKey: "menuQR" },
];
      ...
      <div className="relative" ref={appsRef}>
        ...
        {showApps && (
          <div className="absolute left-0 top-[48px] w-[340px] bg-white rounded-2xl shadow-elevated border border-surface-border animate-drop-in z-50 overflow-hidden">
            ...
            {/* App Grid */}
            <div className="p-3 grid grid-cols-4 gap-1 max-h-[360px] overflow-y-auto">
              {filtered.map((mod) => {
                ...
                if (locked) {
                  return (
                    <Link
                      ...
                      title="Requiere plan PRO — ver planes"
                      className="flex flex-col items-center gap-1.5 p-3 rounded-xl text-center opacity-50 hover:opacity-70 transition-opacity"
                    >
                      <div className="relative">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-sm bg-gray-400">
                          {mod.icon}
                        </div>
                        <Lock size={10} className="absolute -bottom-0.5 -right-0.5 text-white bg-gray-500 rounded-full p-0.5" />
                      </div>
                      <span className="text-[11px] font-semibold leading-tight text-surface-muted">
                        {mod.label}
                      </span>
                    </Link>
                  );
