...
export const DeliveryService = {
  async createPublicOrder(input: {
    ...
    const { sucursalId, items, cliente, metodoPago, cargoEnvio = 0, zonaDelivery, cuponCodigo = null, puntosCanjeados = 0 } = input;
  ...
  async listOrders(scope: DeliveryScope) {
    ...
    return pedidos.map((pedido) => {
      ...
      return {
        ...
        subtotal,
        total: Math.max(0, subtotal + meta.cargoEnvio - meta.descuento),
